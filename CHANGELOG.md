Version 0.1.10 released 02-Jun-2026 (Canberra Australia)
 ...
 - Self-contained subpackage at `src/readabs/extend_history/` — no changes
   to the core `readabs` public API or any existing module.
 ...
 - `extend_series(series_id)` — extend a single series, auto-resolves
   catalogue and sibling. Returns `(series, meta)`.
 ...
 - `find_cat_for_series(series_id)` — look up which catalogue a series
   belongs to.
 - Single-table download optimisation: when the master index has a clean
   table identifier, only that table is downloaded from the current
   catalogue — not the full bundle.
 - Three-tier caching: in-memory master index, module-level historical
   catalogue dict, and on-disk ABS HTTP cache.
